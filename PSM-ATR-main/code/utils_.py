...
def create_dataset(data, voc_size, device, max_visits=-1):
    d_voc_size, p_voc_size, m_voc_size = voc_size[0], voc_size[1], voc_size[2]
...
def create_batches_dict(data, batch_size, voc_size, device, sort=True):
    s_diags, s_procs, s_meds, labels, ids = create_dataset(data, voc_size, device=device)
    tensors = create_batches(s_diags, s_procs, s_meds, labels, ids, batch_size, sort=sort)
    result = {
        'diag_batches':tensors[0],
        'proc_batches':tensors[1],
        'med_batches':tensors[2],
        'label_batches':tensors[3],
        'ids_batches':tensors[4],
        'batch_seq_len':tensors[5],
